...
gene_list <- c('GZMA','GZMB', 'PRF1', 'GNLY','HLA-A', 'HLA-B', 'HLA-C', 'HLA-E', 'HLA-F', 'HLA-G', 'HLA-H',
                 'HLA-DMA', 'HLA-DMB', 'HLA-DOA', 'HLA-DOB', 'HLA-DPA1', 'HLA-DPB1','HLA-DQA1', 'HLA-DQA2',
                 'HLA-DQB1', 'HLA-DRA', 'HLA-DRB1','IFNG', 'IFNGR1', 'IFNGR2', 'IRF1', 'STAT1', 'PSMB9',
                 'CCR5', 'CCL3', 'CCL4', 'CCL5', 'CXCL9', 'CXCL10', 'CXCL11', 'ICAM1', 'ICAM2', 'ICAM3','ICAM4', 'ICAM5', 'VCAM1') 
...
#  prod_non_na <- prod(x[!is.na(x)], na.rm = TRUE)
#  n <- sum(!is.na(x))
#  return(prod_non_na^(1/n))
#}

new_formula <- as.formula(paste("Surv(times, status) ~ ", paste(gene_list, collapse = " + "))) 
fit_cox <- coxph(Surv(times, status) ~ GZMA + GZMB + PRF1 + GNLY + `HLA-A` + `HLA-B` +
                   `HLA-C` + `HLA-E` + `HLA-F` + `HLA-G` + `HLA-H` + `HLA-DMA` +
                   `HLA-DMB` + `HLA-DOA` + `HLA-DOB` + `HLA-DPA1` + `HLA-DPB1` +
                   `HLA-DQA1` + `HLA-DQA2` + `HLA-DQB1` + `HLA-DRA` + 
                   `HLA-DRB1` + IFNG + IFNGR1 + IFNGR2 + IRF1 + STAT1 + PSMB9 + 
                   CCR5 + CCL3 + CCL4 + CCL5 + CXCL9 + CXCL10 + CXCL11 + ICAM1 + 
                   ICAM2 + ICAM3 + ICAM4 + ICAM5 + VCAM1, data = train)
summary(fit_cox)
